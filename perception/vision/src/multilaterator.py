...
class multilaterator:
    ...
    def objects_in_image_cb(self, objects_in_image):
        ...
        if self.stamp > objects_in_image.header.stamp + rospy.Duration(.1):
            rospy.logerr("Image publishing is ahead of object tracking.")
            return
        
        print "Image publishing and persistent_objects_in_image are in sync."
        ...
        for i in objects_in_image.objects:
            ...
            vPrime = R.dot(v)
            self.tracker.add_observation(stamp = self.stamp,features = np.array([t, vPrime]), data = {'id': identifier})
